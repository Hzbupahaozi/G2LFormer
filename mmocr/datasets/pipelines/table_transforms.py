...
class TableResize:
    ...
    def _resize_bboxes(self, results):
        ...
        if 'img_info' in results.keys():
            ...
            # if results['img_info'].get('tr_masks', None) is not None:
            #     tr_bboxes = results['img_info']['tr_masks']
            #     scale_factor = results['scale_factor']
            #     tr_bboxes[..., 0::2] = np.clip(tr_bboxes[..., 0::2] * scale_factor[1], 0, img_shape[1]-1)
            #     tr_bboxes[..., 1::2] = np.clip(tr_bboxes[..., 1::2] * scale_factor[0], 0, img_shape[0]-1)
            #     results['img_info']['tr_masks'] = tr_bboxes
            # else:
            #     raise ValueError('没有使用tr视觉交互模块')                
            if results['img_info'].get('cell_masks', None) is not None:
                cell_masks_col = results['img_info']['cell_masks'][0]
                cell_masks_row = results['img_info']['cell_masks'][1]
                cell_masks_tr = results['img_info']['cell_masks'][2]
                origin_bboxes = results['img_info']['cell_masks'][3]
                scale_factor = results['scale_factor']
                cell_masks_col[..., 0::2] = np.clip(cell_masks_col[..., 0::2] * scale_factor[1], 0, img_shape[1]-1)
                cell_masks_col[..., 1::2] = np.clip(cell_masks_col[..., 1::2] * scale_factor[0], 0, img_shape[0]-1)
                cell_masks_row[..., 0::2] = np.clip(cell_masks_row[..., 0::2] * scale_factor[1], 0, img_shape[1]-1)
                cell_masks_row[..., 1::2] = np.clip(cell_masks_row[..., 1::2] * scale_factor[0], 0, img_shape[0]-1)
                cell_masks_tr[..., 0::2] = np.clip(cell_masks_tr[..., 0::2] * scale_factor[1], 0, img_shape[1]-1)
                cell_masks_tr[..., 1::2] = np.clip(cell_masks_tr[..., 1::2] * scale_factor[0], 0, img_shape[0]-1)
                origin_bboxes[..., 0::2] = np.clip(origin_bboxes[..., 0::2] * scale_factor[1], 0, img_shape[1]-1)
                origin_bboxes[..., 1::2] = np.clip(origin_bboxes[..., 1::2] * scale_factor[0], 0, img_shape[0]-1)                
                # results['img_info']['cell_masks'] = [cell_masks_col, cell_masks_row,origin_bboxes]
                results['img_info']['cell_masks'] = [cell_masks_col, cell_masks_row, cell_masks_tr, origin_bboxes]

            else:
                raise ValueError('没有使用视觉交互模块')
        else:
            # testing phase
            pass
...
class TablePad:
    ...
    def __call__(self, results):
        ...
        # results["cls_bbox"] = results['img_info']["cls_bbox"]   #0302
        # print("pad:")
...
class TableBboxEncode:
    ...
    def __call__(self, results):
        ...
        results["cls_bbox"] = results['img_info']["cls_bbox"]   #0302
        results["cell_masks"] = results['img_info']["cell_masks"]
        results["tr_masks"] = results['img_info']['tr_masks']
        results['num_cell'] = results['img_info']['num_cell']
        results['colrow_masks'] = results['img_info']['colrow_masks']
        # print("tb:",results)
