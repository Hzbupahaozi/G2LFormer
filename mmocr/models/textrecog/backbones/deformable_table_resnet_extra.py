...
class TableResNetExtra(nn.Module):
    ...
    def __init__(self, layers, input_dim=3, gcb_config=None,
        ...
        ### chs 修改，为了使用deformable encoder
        self.maxpool4 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.layer5 = self._make_layer(BasicBlock, 512, layers[4], stride=1, gcb_config=get_gcb_config(gcb_config, 4))
    ...
    def forward(self, x):
        ...
        x = self.maxpool3(x)    #[3,256,60,60]

        x = self.layer3(x)  # [3,512,60,60]
        x = self.conv5(x)
        x = self.bn5(x)
        x = self.relu5(x)
