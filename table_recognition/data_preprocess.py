...
class PubtabnetParser(object):
    def __init__(self, jsonl_path, is_toy=True, split='val', is_pse_preLabel=False, chunks_nums=16):
        self.split = split
        self.raw_img_root = '/media/pci/64fc5b84-602c-4c82-808d-426c880dbfb9/chs/data/dataset/wtw/test/new/'
        self.save_root = '/media/pci/64fc5b84-602c-4c82-808d-426c880dbfb9/chs/data/dataset/wtw/test/'
    ...
    def data_generator(self):
        return json.load(open(self.jsonl_path, 'rb'))
    ...
    def merge_spantoken(self, token_list):
        """
        This function used to merge the common tokens of raw tokens, and reduce the max length.
        eg. merge '<td>' and '</td>' to '<td></td>' which are always appear together.
        :param token_list: [list]. the raw tokens from the json line file.
        ...
        while pointer < length:
            if token_list[pointer] == '<td':
                tmp = token_list[pointer] 
                pointer+=1
                while(token_list[pointer]!="></td>"):
                    pointer+=1
                merge_token_list.append(tmp)
                pointer += 1
            else:
    ...
    def get_structure_alphabet(self):
        ...
        with open(self.alphabet_path_1, 'w') as f:
            for item in tqdm(self.data_generator):
                ...
                merged_spantoken = self.merge_spantoken(merged_token)
                encoded_token = merged_token#merged_token#merged_spantoken
    ...
    def span_num(self,merged_token):
        spannum = []
        max_row,max_col = 30,30
        for i,token in enumerate(merged_token):
            if token == '<td':
                row,col = 1,1
                t1 = merged_token[i+1].split("\"")
                if(t1[0]==" colspan="):
                    col = min(int(t1[1]),max_col)
                elif(t1[0]==" rowspan="):
                    row = min(int(t1[1]),max_row)
                # print(t1)
                t2 = merged_token[i+2].split("\"")
                if(t2[0]==" colspan="):
                    col = min(int(t2[1]),max_col)
                elif(t2[0]==" rowspan="):
                    row = min(int(t2[1]),max_row)
                spannum.append((row,col))
            elif token == '<td></td>':
                spannum.append((1,1))
            elif token in self.empty_bbox_token_reverse_dict.keys():
                spannum.append((1,1))
    ...
    def parse_single_chunk(self, this_chunk, chunks_idx):
        ...
        for item in tqdm(self.data_generator):
            ...
            # print(filename)
            # if(filename!="ch_all_0_091c4361f6703907ae89892e4f0e6309_segR__tR__yf_0.jpg"): continue
            ...
            # encoded_token = self.insert_empty_bbox_token(merged_token, cells)
            #span num
            spannum = self.span_num(merged_token)
            # print("done")
            merged_spantoken = self.merge_spantoken(merged_token)
            # encoded_token =merged_spantoken
            encoded_token = merged_token#merged_spantoken #merged_token
            ...
            # print(cell_nums,cell_count,len(spannum))
            try:
                assert cell_nums == cell_count ==len(spannum)
                for i,cell in enumerate(cells):
                    if 'bbox' not in cell.keys():
                        bbox_line = ','.join([str(0) for _ in range(4)]) + ","+spannum[i][0]+","+spannum[i][1]+ '\n'
                    else:
                        bbox_line = ','.join([str(b) for b in cell['bbox']]) 
                        # print(bbox_line)
                        bbox_line = bbox_line+ ","+str(spannum[i][0])+","+str(spannum[i][1])+ '\n'
        ...
        #         continue
            ...
            # recognition_filename = filename.split('.')[0] + '_' + str(idx) + '.jpg'ss
...
if __name__ == '__main__':
    ...
    # parse train
    nproc = 1
    jsonl_path = r'/media/pci/64fc5b84-602c-4c82-808d-426c880dbfb9/chs/data/dataset/wtw/wtw_test.json'
    # jsonl_path = r'/home/Dataset/mmocr_HWL_recognition/hwl.json'
    parser = PubtabnetParser(jsonl_path, is_toy=False, split='test', is_pse_preLabel=False, chunks_nums=nproc)

    # multiprocessing
    start_time = time.time()
    filenames, count = parser.get_filenames(split='test')
    ...
    # img_chunks = arser.divide_img(filenames)
