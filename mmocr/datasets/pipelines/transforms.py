...
class RandomRotatePolyInstances:
    ...
    def rotate_img(self, img, angle, canvas_size):
        ...
        if self.pad_with_fixed_color:
            ...
            # img_cut = img_cut.permute(1,2,0)
            # print("type:",type(img_cut),img_cut.shape)                                  
    ...
    def __call__(self, results):
        ...
        if np.random.random_sample() < self.rotate_ratio:
            ...
            # rotate polygons
            # print("POL",results.get('mask_fields', []))
            for key in results.get('mask_fields', []):
                print("len:", len(results[key].masks) )
                if len(results[key].masks) == 0:
                    continue
                masks = results[key].masks
                rotated_masks = []
